...
- **With no window manager the window ignores its requested size** (a `default_width=700` window is allocated 640 under Xvfb), so tests force the width with an explicit `allocate()` *after* present.
