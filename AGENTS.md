...
**Testing:** the tests are genuinely headless — `Gtk.TextBuffer` manipulation and widget construction only need GTK initialized, not a realized/displayed window. **No `xvfb-run` wrapper is needed anywhere, CI included**; this was measured by running the suite with `DISPLAY` and `WAYLAND_DISPLAY` both unset, not assumed. Tests assert tag placement on parsed-and-rendered buffers (e.g., parse `"hello **world**"`, locate `"world"`, assert it has the `strong` tag) — this is the pattern to follow for new renderer coverage rather than snapshot-testing rendered output.
